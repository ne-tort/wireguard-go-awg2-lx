...
// EgressProvider anchors StdNetBind UDP traffic through sing-tun's
// UDPEgressPool (TUN auto-redirect / exclude-interface). Implemented by
// github.com/sagernet/sing-tun.UDPEgressPool.
...
again:
	...
	if s.egressProvider != nil {
		...
		fns = append(fns, func(bufs [][]byte, sizes []int, endpoints []Endpoint) (int, error) {
			...
			sizes[0] = dataLength
			// Same gate as receiveIP: never strip bytes 1-3 for AmneziaWG
			// (skipReserved); plain WG/WARP still clears WARP reserved on rx.
			if !s.skipReserved && dataLength > 3 {
